...
def simulate_mac_with_fading(snr_db_range,no_of_users,nvps,rnd_seed):
    ...
    for snr_db in snr_db_range:
        ...
        channel_coeff=np.random.randn(no_of_users,nvps)
        
        pre_processed_signal=pre.pre_process(msg,channel_coeff,snr_db)
        # print("pre processed:\n",pre_processed_signal)
        
        # Pass the transmitted symbols through the AWGN channel to get received symbols
        received_signal = ch.awgn_mac_with_channel_coeff(pre_processed_signal, snr_db,channel_coeff,no_of_users,nvps,rnd_seed)  # Received symbols: channel output = symbols + noise
